...
import { Theme, themeStore } from "../../ThemeStore";
...
export default function ThemeIcon(props: Props) {
  ...
  return (
    <svg
      ...
      class={`${props.class} ${styles.ThemeIcon}`}
    ...
    >
      <Switch fallback={DefaultDarkModeIcon}>
        <Match when={theme() === Theme.DefaultDark}>
          {DefaultDarkModeIcon}
        </Match>
        <Match when={theme() === Theme.DefaultLight}>
